...
function install {
	...
	*) fail "unknown os: $(uname -s)";;
	...
	"linux_arm")
		...
		;;
	"linux_amd64")
		URL="https://github.com/sharkdp/fd/releases/download/v9.0.0/fd-v9.0.0-i686-unknown-linux-gnu.tar.gz"
		FTYPE=".tar.gz"
		;;
	"darwin_amd64")
		URL="https://github.com/sharkdp/fd/releases/download/v9.0.0/fd-v9.0.0-x86_64-apple-darwin.tar.gz"
		FTYPE=".tar.gz"
		;;
	*) fail "No asset for platform ${OS}-${ARCH}";;
	...
	if [[ $FTYPE = ".gz" ]]; then
		...
		bash -c "$GET $URL" > "fd_${OS}_${ARCH}" || fail "download failed"
	else
