...
-- edit lsp-config
set('n', 'gel', '<cmd>e ~/.config/nvim/lua/waylonwalker/lsp-config.lua<CR>')
-- edit plugins list
...
-- edit drafts
set('n', 'geid', "<cmd>Telescope find_files find_command=markata,list,--map,path,--filter,status=='draft',--sort,date,--reverse,--fast<cr>")
-- edit tils
set('n', 'geil', "<cmd>Telescope find_files find_command=markata,list,--map,path,--filter,templateKey=='til',--sort,date,--reverse,--fast<cr>")
-- edit gratitude journals
set('n', 'geig', "<cmd>Telescope find_files find_command=markata,list,--map,path,--filter,templateKey=='gratitude',--sort,date,--fast<cr>")
--  set('n', 'geik', '<cmd>Telescope find_files find_command=markata,list,--map,path,--filter,'kedro' in tags,--sort,date,--reverse<cr>')
--  edit draft posts
set('n', 'geid', "<cmd>lua require('telescope.builtin').find_files({find_command={'markata', 'list', '--map', 'path', '--filter', 'status=='draft' and templateKey!='gratitude'', '--sort', 'date', '--fast'}})<cr>")
-- edit kedro posts
set('n', 'geik', "<cmd>lua require('telescope.builtin').find_files({find_command={'markata', 'list', '--map', 'path', '--filter', ''kedro' in tags', '--sort', 'date', '--reverse'}})<cr>")
-- edit draft kedro
set('n', 'geidk', "<cmd>lua require('telescope.builtin').find_files({find_command={'markata', 'list', '--map', 'path', '--filter', ''kedro' in tags and status != 'published'', '--sort', 'date', '--reverse'}})<cr>")


-- go to the internet
set('n', 'gow', "<cmd>lua os.execute('xdg-open https://waylonwalker.com/' .. vim.api.nvim_buf_get_name(0):match(\"^.+/(.+)$\"):gsub('.md', '') .. '/ > /dev/null 2>&1')<cr>")
set('n', 'goo', "<cmd>Telegraph 'xdg-open \"{cWORD}\"'<cr>")

set('n', '<leader>e', ':e %:h<C-Z>')


set('n', 'gpu', "<cmd>lua require'telegraph'.telegraph({how='tmux_popup', cmd='nvim --headless -c \"autocmd User PackerComplete quitall\" -c \"PackerSync\"'})<cr>")
...
set('n', '<leader><leader>s', '<cmd>lua require\'telegraph\'.telegraph({cmd=\'pipx run --spec git+https://github.com/waylonwalker/lookatme lookatme {filepath} --live-reload --style gruvbox-dark\', how=\'tmux\'})<CR>')
-- lookatme slides for the current file in a tmux popup
set('n', '<leader><leader>S', '<cmd>lua require\'telegraph\'.telegraph({cmd=\'pipx run --spec git+https://github.com/waylonwalker/lookatme lookatme {filepath} --live-reload --style gruvbox-dark\', how=\'tmux_popup\'})<CR>')
-- visidata the current word in a tmux session
set('n', '<leader><leader>vd', '<cmd>lua require\'telegraph\'.telegraph({cmd=\'pipx run visidata {cWORD}\', how=\'tmux\'})<CR>')
-- open a manpage
...
set('n', '<leader><leader>r', '<cmd>lua require\'telegraph\'.telegraph({cmd=\'zsh -c "ipython {filepath} -i"\', how=\'tmux_popup\'})<CR>')
-- run zsh in a popup
set('n', '<leader><leader>z', '<cmd>lua require\'telegraph\'.telegraph({cmd=\'zsh\', how=\'tmux_popup\'})<CR>')
-- set the title 
set('n', '<leader>t', '<cmd>lua require\'telegraph\'.telegraph({how=\'execute\', cmd=\'echo "{cline}" > ~/.config/title/title.txt\'})<cr>')
